...
- **Write claim.** Before your first write to a repository, read `_tools/.agents/runtime/claims.md` and add a line «repo-or-path · ticket · who · taken · expires» (expiry 2h, ONE path per line); done — remove it. Someone else's live overlapping line — stop and ask the human; an expired one is free, delete it. There is no "I am alone here" exemption: a session cannot know that — which is exactly the root of I-032; the price of being wrong is one line that expires by itself. Format and the one-line check — the `pf-handoff` skill (a separate distribution), `references/context-rules.md`.
