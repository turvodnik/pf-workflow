...
- **Risk-based gating** (`gate = auto`): a gate after every batch where batches build on each other or touch executable artifacts (code, configs, schemas, scripts); a single gate per milestone where batches are independent and cheap to fix (texts, docs, standalone pages). An independent reviewer subagent re-verifies every criterion with evidence against the diff; the executor's "Result" is never taken on faith. Always mandatory: a gate before a dependent batch, a gate before any external action, and the final whole-task review.
