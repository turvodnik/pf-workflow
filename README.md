...
Run the test suite locally with `bash tests/run.sh` (bash 3.2+, no dependencies beyond coreutils and git; ShellCheck, python3/PyYAML and actionlint are used when present, skipped with a reason otherwise). The same command runs in CI on every push and pull request ([`.github/workflows/tests.yml`](.github/workflows/tests.yml)). It never invokes the real Codex CLI or touches the network — a fixture stands in for `codex` throughout. [`tests/known-failures-T017.txt`](tests/known-failures-T017.txt) tracks cases where an un-synced copy in this repo is knowingly red against canon — empty right now, since this release is freshly synced.
